...
NTRT.LimbAfflictions = {-- same deal
...
NTRT.RemovedAfflictions = { -- afflictions that are periodically removed from robots
    "huskinfection",
    "bleeding",
    "bleedingnonstop",
    "nausea",
...
Timer.Wait(function()
    ...
    NT.UpdateHuman = function(character)
        ...
        local charIsRobot = NTRT.IsRobot(character)
    
        -- fetch all the current affliction data
        for identifier,data in pairs(NT.Afflictions) do
            if not charIsRobot or TableContains(NTRT.Afflictions,identifier) then
        ...
        for identifier,data in pairs(NT.CharStats) do
            if not charIsRobot or TableContains(NTRT.CharStats,identifier) then
                if data.getter ~= nil and (not charIsRobot or NTRT.CharStats[identifier].leaveempty == nil) then charData.stats[identifier] = data.getter(charData)
                else charData.stats[identifier] = data.default or 1 end
            end
        end
        -- update non-limb-specific afflictions
        for identifier,data in pairs(NT.Afflictions) do
            if not charIsRobot or (TableContains(NTRT.Afflictions,identifier) and NTRT.Afflictions[identifier].getonly == nil) then
        ...
        local function FetchLimbData(type)
            ...
            for identifier,data in pairs(NT.LimbAfflictions) do
                if not charIsRobot or TableContains(NTRT.LimbAfflictions,identifier) then
        ...
        local function UpdateLimb(type)
            local keystring = tostring(type).."afflictions"
            for identifier,data in pairs(NT.LimbAfflictions) do
                if not charIsRobot or (TableContains(NTRT.LimbAfflictions,identifier) and NTRT.LimbAfflictions[identifier].getonly == nil) then
                    if data.update ~= nil then
    ...
    NTC.AddHumanUpdateHook(NTRT.UpdateRobot)
end,1)


function NTRT.UpdateRobot(character)
    if not NTRT.IsRobot(character) then return end
    ...
    for identifier in NTRT.RemovedAfflictions do
...
Timer.Wait(function()
    ...
    NTC.AddPreHumanUpdateHook(NTRT.UpdateHuman)

end,1000)

function NTRT.UpdateHuman(character)
    ...
    local function updateLimb(character,limbtype) 
        ...
        if isRobot then
            if materialloss >= 99 and not NT.LimbIsAmputated(character,limbtype) then
...
function NTRT.ConvertDamageTypes(character,limbtype)
